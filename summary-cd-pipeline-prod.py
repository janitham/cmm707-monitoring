...
def updatingGitOpsRepo():
    ...
    BUILD_VERSION = executeCommand("kubectl get deploy summary -n test -o jsonpath='{.metadata.labels.version}'", GITOPS_SYNC_REPO).decode("utf-8").replace("'", "")
    print(BUILD_VERSION)
...
def waitUntilSync():
    ...
    BUILD_VERSION = executeCommand("kubectl get deploy summary -n test -o jsonpath='{.metadata.labels.version}'", GITOPS_SYNC_REPO).decode("utf-8").replace("'", "").strip()

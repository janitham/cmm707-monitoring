...
class Tester:
    ...
    def createdSummary(self):
        url = "http://localhost:"+DEPLOYMENT_PORT+"/summary/created"
        response = requests.request("GET", url, headers=headers, data={})
        if response.status_code != 200:
            raise "Error summary created"
        
    def updatedSummary(self) :
        url = "http://localhost:"+DEPLOYMENT_PORT+"/summary/updated"
        response = requests.request("GET", url, headers=headers, data={})
        if response.status_code != 200:
            raise "Error summary updated"
        
    def deletedSummary(self):
        url = "http://localhost:"+DEPLOYMENT_PORT+"/summary/deleted"
        response = requests.request("GET", url, headers=headers, data={})
        if response.status_code != 200:
